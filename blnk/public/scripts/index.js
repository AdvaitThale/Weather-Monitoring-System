...
function epochToJsDate(epochTime){
  ...
  const MAXATElement = document.getElementById("MAXAT");
  const MAXHElement = document.getElementById("MAXH");
  const MAXIElement = document.getElementById("MAXI");
  const MAXLElement = document.getElementById("MAXL");
  const MAXCTElement = document.getElementById("MAXCT");
  const MAXCPElement = document.getElementById("MAXCP");
  const MAXVIBElement = document.getElementById("MAXVIB");
  const MAXTElement = document.getElementById("MAXT");
  const MAXVXElement = document.getElementById("MAXVX");
  const MAXVYElement = document.getElementById("MAXVY");
  const MAXVZElement = document.getElementById("MAXVZ");
  ...
  const setupUI = (user) => {
    if (user) {
      ...
      chartRef.on('value', snapshot =>{
        ...
        chartMAXAT = createMAXATChart();
        chartMAXH = createMAXHChart();
        // chartP = createPressureChart();
        // Update the charts with the new range
        // Get the latest readings and plot them on charts (the number of plotted readings corresponds to the chartRange value)
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          var jsonData = snapshot.toJSON(); // example: {MAXAT: 25.02, MAXH: 50.20, pressure: 1008.48, timestamp:1641317355}
          // Save values on variables
          var MAXAT = jsonData.MAXAT;
          var MAXH = jsonData.MAXH;
          // var pressure = jsonData.pressure;
          var timestamp = jsonData.timestamp;
          // Plot the values on the charts
          plotValues(chartMAXAT, timestamp, MAXAT);
          plotValues(chartMAXH, timestamp, MAXH);
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var timestamp = jsonData.timestamp;
        // Update DOM elements
        MAXATElement.innerHTML = MAXAT;
        MAXHElement.innerHTML = MAXH;
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var timestamp = jsonData.timestamp;
        // Update DOM elements
        var gaugeT = createMAXATGauge();
        var gaugeH = createMAXHGauge();
        gaugeT.draw();
        gaugeH.draw();
        gaugeT.value = MAXAT;
        gaugeH.value = MAXH;
      ...
      function createTable(){
        ...
        dbRef.orderByKey().limitToLast(100).on('child_added', function(snapshot) {
          if (snapshot.exists()) {
            ...
            var MAXAT = jsonData.MAXAT;
            var MAXH = jsonData.MAXH;
            var MAXI = jsonData.MAXI;
            var MAXL = jsonData.MAXL;
            var MAXCT = jsonData.MAXCT;
            var MAXCP = jsonData.MAXCP;
            var MAXVIB = jsonData.MAXVIB;
            var MAXT = jsonData.MAXT;
            var MAXVX = jsonData.MAXVX;
            var MAXVY = jsonData.MAXVY;
            var MAXVZ = jsonData.MAXVZ;
            var timestamp = jsonData.timestamp;
            var content = '';
            content += '<tr>';
          ...
          if (snapshot.exists()) {
            ...
            reversedList.forEach(element =>{
              if (firstTime){ // ignore first reading (it's already on the table from the previous query)
                ...
                var MAXAT = element.MAXAT;
                var MAXH  = element.MAXH;
                var MAXI  = element.MAXI;
                var MAXL  = element.MAXL;
                var MAXCT = element.MAXCT;
                var MAXCP = element.MAXCP;
                var MAXVIB  = element.MAXVIB;
                var MAXT  = element.MAXT;
                var MAXVX = element.MAXVX;
                var MAXVY = element.MAXVY;
                var MAXVZ = element.MAXVZ;
